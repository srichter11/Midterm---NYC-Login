//
//  VacantLandViewController.swift
//  Midterm - NYC Login
//
...
class VacantLandViewController: UIViewController, MKMapViewDelegate {
    ...
    func mapView(mapView: MKMapView, viewForAnnotation annotation: MKAnnotation) -> MKAnnotationView? {
        ...
        if let annotation = annotation as? lotLocation {
            if annotation.owner != "" {
    ...
    func loadVacantLandData() {
        mapView.removeAnnotations(mapView.annotations)
        let dataURL = "https://raw.githubusercontent.com/srichter11/596-Acres-Data/master/Living%20Lots%20NYC%20lots%202016-04-16.json"
        
        Alamofire.request(.GET, dataURL).responseData { response in
            if let data = response.data {
                let json = JSON(data: data)
                let locations = json["features"].arrayValue
                for location in locations {
                    
                    let coordinate = CLLocationCoordinate2DMake(location["geometry"]["coordinates"][1].doubleValue, location["geometry"]["coordinates"][0].doubleValue)
                    
                    let locationAnnotation = lotLocation(coordinate: coordinate, title: location["properties"]["known_use"].stringValue, owner: location["properties"]["owner"].stringValue)
...
class lotLocation:NSObject, MKAnnotation {
    var coordinate: CLLocationCoordinate2D
    var title: String?
    var owner: String?
